...
def make_data_loaders():
    train_loader = make_data_loader(config.get('poisoned_train_dataset'), tag='train')

    val_loader = make_data_loader(config.get('val_dataset'), tag='val')

    poisoned_val_loader = make_data_loader(config.get('poisoned_val_dataset'), tag='poisoned_val')

    return train_loader, val_loader, poisoned_val_loader
...
def main(config_, save_path, args):
    ...
    train_loader, val_loader, poisoned_val_loader = make_data_loaders()
    ...
    fozen_count = 0
    trainable_count_decoder = 0
    trainable_count_adapter = 0
    trainable_count_others = 0
    for name, para in model.named_parameters():
        if "image_encoder" in name and "prompt_generator" not in name:
            para.requires_grad_(False)
            fozen_count += para.numel()
        else:
            if "mask_decoder" in name:
            # print(name)
                trainable_count_decoder += para.numel()
            else:
                if "lightweight_mlp" in name:
                    trainable_count_adapter += para.numel()
                else:
                    trainable_count_others += para.numel()
    print(fozen_count, trainable_count_decoder, trainable_count_adapter)
    input(trainable_count_others)
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        if (epoch_val is not None) and (epoch % epoch_val == 0):
            ...
            print(eval_psnr(poisoned_val_loader, model,
                  eval_type=config.get('eval_type')))
